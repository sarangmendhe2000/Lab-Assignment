package com.demo.TestEmployee;
import java.util.List;
import java.util.Scanner;
import java.util.Set;

import com.demo.beans.Employee;
import com.demo.service.EmployeeService;
import com.demo.service.EmployeeServiceImpl;

public class TestEmployeeManagement {

	public static void main(String[] args) {
		
		Scanner sc=new Scanner(System.in);
		EmployeeService eservice = new EmployeeServiceImpl();
		
		int choice=0;
		do {
			System.out.println("1.Add New Employee");
			System.out.println("2.Display Details");
			System.out.println("3.Search by Id ");
			System.out.println("4.Search by Name");
			System.out.println("5. Search By Salary");
			System.out.println("6. Update Salary");
			System.out.println("7.Delete By Id");
			System.out.println("8.Delete By Salary");
			System.out.println("9.Sort By Salary");
			System.out.println("10. Sort By Name");
			System.out.println("11. Sort By Id");
			System.out.println("12. Exit");
			System.out.println("Enter choice");
			choice=sc.nextInt();
		switch(choice) {
		
		case 1:
		
		boolean status=eservice.addNewEmployee();
		if(status){
			
			System.out.println("Added Successfully");
		}else
		{
			System.out.println("Failed to add");
		}
		break;
		case 2:
			Set<Employee> eset=eservice.displayAll();
			eset.forEach(System.out::println);
		
		break;
		
		case 3:
			System.out.println("Enter Id for Search");
			int id=sc.nextInt();
			Employee es=eservice.displayById(id);
			if(es!=null) {
				System.out.println(es);
			}else {
				
				System.out.println("Id Not Found");
			}
			
			break;
			
		case 4:
			System.out.println("Enter name to Search : ");
			String name = sc.next();
			
			Set<Employee> ese = eservice.searchByName(name);
			
			if(!ese.isEmpty())
			{
				ese.forEach(System.out::println);
			
			}
			else
			{
				System.out.println("Name not found ");
			}
			
			break;
			
		case 5:
			
			System.out.println("Enter Salary for Search Employee");
			
			double sal = sc.nextDouble();
			
			Set<Employee> setSal = eservice.searchBySalary(sal);
			
			if(! setSal.isEmpty())
			{
				setSal.forEach(System.out::println);
			}
			else
			{
				System.out.println("Salary Not Found");
			}
			
			break;
			
		case 6:
			System.out.println("Enter Id For Update");
			int uid=sc.nextInt();
			System.out.println("Enter  to update Salary");
			double sall=sc.nextDouble();
			boolean statuss=eservice.updateBySalary(uid,sall);
			if(statuss) {
				System.out.println("Update successfully");
			}else
			{
				System.out.println("Error:");
			}
			
			break;
			
		case 7:
			System.out.println("Enter ID for Delete");
			int del_id= sc.nextInt();
			boolean status2 = eservice.deleteById(del_id);
			
			if(status2)
			{
				System.out.println("Deleted Successfully ! ");
			}
			else
			{
				System.out.println("Error ! Try Again");
			}
			
			break;
		case 8:
			System.out.println("Enter Salary for Delete");
			
			double del_sal = sc.nextDouble();
			
			boolean status3 = eservice.deleteBySalary(del_sal);
			
			if(status3)
			{
				System.out.println("Deleted Successfully ! ");
			}
			else
			{
				System.out.println("Error , please Try Again");
			}
			
			break;
		case 9:
			List <Employee> elst=eservice.sortBysal();
			elst.forEach(System.out::println);
			break;
		case 10:
			List<Employee> elist=eservice.sortByName();
			elist.forEach(System.out::println);
			break;
			
		case 11:
			
			Set<Employee> srt = eservice.sortById();
			
			srt.forEach(System.out :: println);
			
			
			break;
		case 12:
			System.out.println("Thankyou for visiting...");
			break;
		default:
			System.out.println("Invalid choice");
	
		}
		
	
		
		
			
		
		}while(choice!=12);
		
		
		sc.close();
	}
}
