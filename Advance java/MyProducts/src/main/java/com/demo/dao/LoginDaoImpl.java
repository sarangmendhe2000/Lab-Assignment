...
public class LoginDaoImpl implements LoginDao {
	...
	static {
		...
		try {
			...
			addUser = conn.prepareStatement("Insert into registration (uname, email, password) values ( ? , ? , ?)");
	...
	public boolean addNewUser(User myUser) {
		...
		try {
			addUser.setString(1, myUser.getName());
			addUser.setString(2, myUser.getEmail());
			addUser.setString(3, myUser.getPassword());
